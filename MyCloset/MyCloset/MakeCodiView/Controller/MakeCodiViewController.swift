...
class MakeCodiViewController: UIViewController {
    ...
    private func distributeImages() {
        print(SelectedImageSingleton.shared.selectedImageSet)
        let keys = SelectedImageSingleton.shared.selectedImageSet.keys
        for key in keys {
            switch key {
            case "cap":
                self.capView.image = SelectedImageSingleton.shared.selectedImageSet[key]
            case "outer":
                self.outerView.image = SelectedImageSingleton.shared.selectedImageSet[key]

            case "top":
                self.topView.image = SelectedImageSingleton.shared.selectedImageSet[key]

            case "bottom":
                self.bottomView.image = SelectedImageSingleton.shared.selectedImageSet[key]

            case "shoes":
                self.shoesView.image = SelectedImageSingleton.shared.selectedImageSet[key]

            case "bag":
                self.bagView.image = SelectedImageSingleton.shared.selectedImageSet[key]

            case "acc":
                self.accView.image = SelectedImageSingleton.shared.selectedImageSet[key]

            case "socks":
                self.socksView.image = SelectedImageSingleton.shared.selectedImageSet[key]
    ...
    private func setView() {
        ...
        imageViews.forEach{$0.backgroundColor = UIColor(named: "codiBackground"); $0.translatesAutoresizingMaskIntoConstraints = false;$0.contentMode = .scaleAspectFit}
    }
...
//        let imageCount = CodiSingleton.shared.codiImages.count
//        CodiSingleton.shared.codiImages.append(containerImageView.asImage())
//        if CodiSingleton.shared.codiImages.count > imageCount {
            var fileNum = 0
            ...
            codiRef.listAll { (StorageListResult, Error) in
                ...
                if Error == nil {
                    fileNum = StorageListResult.items.count
                    print(StorageListResult.items)
                    codiRef.child("codiSet"+"\(fileNum)"+".jpeg").putData(data!, metadata: meta) { (_, error) in
                                          if error == nil {
                                           print("코디셋 업로드 성공")
